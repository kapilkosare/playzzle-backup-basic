...
export async function updateCategoryOrder(name: string, newOrder: number): Promise<{ success: boolean; message: string }> {
    await verifyAdmin();
    const db = getFirestore(app);
    try {
        await db.collection('categories').doc(name).update({ displayOrder: newOrder });
        revalidatePath('/puzzles');
        revalidatePath('/super-admin/puzzles');
